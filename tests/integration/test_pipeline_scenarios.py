...
def mock_model_step(
    ...
    with tracker.start_run(run_id, model=step_name, config=config, inputs=resolved_inputs, cache_mode=cache_mode) as t:
        if t.is_cached:
            assert len(t.current_consist.outputs) > 0, f"Step {step_name}: Cached run missing outputs!"
...
def test_cache_overwrite_mode(tracker_setup, ctx):
    ...
    # ASSERTIONS
    assert "A" in ctx.execution_log, "Step A should have executed despite matching previous run"
    ...
    with tracker.engine.connect() as conn:
        ...
        count = pd.read_sql("SELECT count(*) as c FROM run WHERE model_name='A'", conn).iloc[0]['c']
        assert count == 2
...
def test_ghost_mode_ingested(tracker_setup, ctx):
    ...
    with tracker.engine.connect() as conn:
        ...
        if not target_table: raise ValueError(f"Table not found in {tables}")
