...
def tracker_setup(tmp_path: Path):
    """
    Pytest fixture that sets up a Tracker instance and a dummy input file for caching tests.

    This fixture creates a temporary run directory and a DuckDB database file.
    It also generates a simple `source_data.csv` file, which serves as an input
    artifact for the test runs. This setup ensures an isolated and consistent
    environment for testing Consist's caching and lineage features.
    ...
    Tuple[Tracker, Path]
        A tuple containing:
        - `Tracker`: An initialized Consist `Tracker` instance.
        - `Path`: The path to the dummy input file (`source_data.csv`).
    """
