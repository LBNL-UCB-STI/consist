    ...
    def batch_artifact_writes(self) -> Iterator[None]:
        ...
        try:
            ...
            if pending_json_flush:
                with track_begin_run_phase("input_binding.batch_flush_json"):
                    self._flush_json_now()
            if pending_artifacts:
                with track_begin_run_phase("input_binding.batch_sync_artifacts"):
                    self._sync_artifacts_now(pending_artifacts)
            if pending_artifact_facet_bundles:
                with track_begin_run_phase("input_binding.batch_persist_facets"):
